...
function gcinclude.GetMoon()
    local timestamp = gcinclude.GetTimestamp();
...
function gcinclude.GetTimestamp()
...
function gcinclude.HandleCommands(args)
	...
	elseif (args[1] == 'tavring') then
		gcinclude.DoTavRing();
	elseif (args[1] == 'treat') then
		gcinclude.DoTreat();
	elseif (args[1] == 'mandy') then
		gcinclude.DoMandy();
...
function gcinclude.DoTreat()

	AshitaCore:GetChatManager():QueueCommand(1, '/lac equip main "Treat Staff II"');
	local function usermain()
		AshitaCore:GetChatManager():QueueCommand(1, '/item "Treat Staff II" <me>');
	end
...
function gcinclude.DoMandy()
	...
	AshitaCore:GetChatManager():QueueCommand(1, '/lac equip body "Mandragora Suit"');
	local function usebody()
		AshitaCore:GetChatManager():QueueCommand(1, '/item "Mandragora Suit" <me>');
	end
	usebody:once(31); -- Execute after 31s?
